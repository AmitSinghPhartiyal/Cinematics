     	 ...
     	 return (
			...
			return(
				<View style = {{ flex:1, flexDirection:'column',}}>
					<View style = {{ flex:0.6,position:'relative',flexDirection:'column',}}>
						<View style = {{flex:0.2,flexDirection:'row',position:'absolute',backgroundColor:'transparent',zIndex:100,padding:10}}>
							<TouchableOpacity style = {{flex:0.6}} onPress = {() => Actions.Home()}>
							...
							<TouchableOpacity style = {{flex:0.15,justifyContent:'center',alignItems:'center'}} onPress={this.onOpen.bind(this)}>
								<Icon name = "share-alt" style={{color:'#fff',fontSize:25}}/>
							</TouchableOpacity>
						...
						<View style= {{ flex:0.6,backgroundColor:'#FEFCFD',}}>
							<Image  source = {{uri :imgPath + this.props.movie.backdrop_path}} 
									style = {{ flex:1,position:'relative'}}
									indicator={ActivityIndicator}
							/>
						</View>
						<View style = {{flex:0.4,	flexDirection:'row',backgroundColor:'#12130F',}}>
							<View style = {{ flex:0.4,}}></View>
							<View style = {{ flex:0.6,justifyContent:'center',}}>
								<View style = {{ flex:1,flexDirection:'column',padding:10}}>
									<View style = {{flex:0.4,flexDirection:'row'}}>
										<View style = {{ flex:0.15,alignItems:'flex-start',justifyContent:'center',marginLeft:5,}}>
											<Text style = {{color:'#7D7D7D',}}>{!this.props.movie.adult? 'N/A' : 18 }</Text>
										</View>
										<View style = {{ flex:0.1, alignItems:'center',justifyContent:'center',}}>
											<Icon name = "circle" style = {{color:'#7D7D7D'}} />
										</View>
										<View style= {{ flex:0.25,alignItems:'center',justifyContent:'center',}}>
											<Text style = {{color:'#7D7D7D',padding:5,}}>{this.props.movie.release_date?new Date(this.props.movie.release_date).getFullYear():new Date(this.props.movie.first_air_date).getFullYear()}</Text>
										</View>
										<View style = {{ flex:0.1, alignItems:'center',justifyContent:'center',}}>
											<Icon name = "circle" style = {{color:'#7D7D7D'}} />
										</View>
										<View style = {{ flex:0.45,marginLeft:5,justifyContent:'center',alignItems:'flex-start'}}>
											<Text style = {{color:'#7D7D7D'}}>{
												this.props.detail && this.props.detail.runtime?
												Math.floor(this.props.detail.runtime/60) +' hr ' +this.props.detail.runtime % 60 +'min':'N/A'}</Text>
										</View>
									</View>
									<View style = {{flex:0.4,}}>
										<Text numberOfLines={2} style = {{ fontSize:20,color:"#fff",fontWeight:'bold',marginLeft:4,}}>{this.props.movie.original_title?this.props.movie.original_title:this.props.movie.name}</Text>
									</View>
									<View style = {{flex:0.4,}}>
										<FlatList
											...
											if(this.props.detail.genres){
												return(
														<Text style={{ color: '#7D7D7D',alignItems:'flex-start' }}> { item.name } </Text>				
													)
											} else {
						...
						<Image 	source = {{uri :imgPath + this.props.movie.poster_path}} 
								style = {{ width:width*0.3,height:width*0.5,position:'absolute',marginTop:height*0.3,marginLeft:width*0.05,}}
								indicator={ActivityIndicator} 
						/>
					</View>
					<View style = {{flex:0.4,}}>
						<ScrollableTabView
							...
							<Info tabLabel = "INFO" info = { this.props.movie } detail = {this.props.detail}/>
