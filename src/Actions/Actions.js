...
export function getMoviesTrailors(movieId) {
	return(dispatch) => {
		fetch('https://api.themoviedb.org/3/movie/' +  movieId + '/videos?api_key=55032e2af54d05c1326b26b0bf830b60&language=en-US')
			.then((response)=> response.json())
			.then((responseJson) => {
				console.log('Trailors:',responseJson)
...
export function changeDrawer(changeValue) {
	return(dispatch) => {
                console.log("changeSidebar",changeValue);
